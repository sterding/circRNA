###############################################
## Rscript to run circulization QTL using SVA
## Author: Xianjun Dong
## Date: 2017-Jul-31
## Version: 0.0
## Usage: bsub -q big -n 2 -R 'rusage[mem=10000]' Rscript ~/neurogen/pipeline/RNAseq/modules/_SVA.cQTL.R HCILB_SNDA
###############################################
require(reshape2)
require(MatrixEQTL)
require(RCurl)
...
wd=paste0("~/projects/circRNA/results/cQTL/",SAMPLE_GROUP);
...
geneloc="~/projects/circRNA/results/Merge_circexplorer_BC.annotation.loci.txt";

if(file.exists("data.RData")) load("data.RData") else{
  
  message("## loading expression data...")
  ######################
  expr = readRDS("~/projects/circRNA/data/Merge_circexplorer_BC.annotation.bed14.cRatio.circRNA_norm_filtered.rds")
  ...
  ######################
  if(file.exists("~/projects/circRNA/results/eQTL/HCILB_SNDA/snps.rds")) snps=readRDS("~/projects/circRNA/results/eQTL/HCILB_SNDA/snps.rds") else{
    ...
    saveRDS(snps,file = "~/projects/circRNA/results/eQTL/HCILB_SNDA/snps.rds")
  }
...
# par(mfrow=c(2,1))
# hist(apply(expr,1,mean), breaks=100, xlab="cRatio", main="cRatio distribution before adjustment")
# hist(apply(residuals,1,mean), breaks=100, xlab="cRatio", main="cRatio distribution after adjustment")
# dev.off()
