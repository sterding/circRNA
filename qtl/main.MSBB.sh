...
for i in `seq 1 22`; do echo $i;  cat leafcutter/sqtl_tmpdir/leafcutter_perind.counts.gz.qqnorm_chr$i | sed 's/.accepted_hits.bam//g;s/.Aligned.out.bam//g;s/HC_//g;s/ILB_//g;s/_SNDA_[1-9]_rep[12]//g' | awk '{OFS="\t"; s=0; for(i=5;i<=NF;i++) if($i>0) s++; if(NR==1 || s>=1) print}' > circRNA.PSI.chr$i.bed & done
