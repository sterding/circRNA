...
gene_or_HTNE   = args[4]  # <gene|HTNE|circRNA>
...
colnames(eqtl) = c('SNPid_illumina', 'gene', 'beta', 't.stat', 'p.value', 'FDR', 'SNP.pos','SNP.chr')
...
if(grepl("circRNA|circularRNA", gene_or_HTNE)){
  annotation=read.table("~/projects/circRNA/data/Merge_circexplorer_BC.annotation.bed14", stringsAsFactors =F, header=T); 
  result = cbind(result, assocaitedRNA_hostgene=annotation$geneName[match(result$assocaitedRNA, annotation$ID)])
  result$assocaitedRNA_coord=result$assocaitedRNA
}
# add postion of SNPs
