...
df = Merge_circexp_norm; dim(df)
#df = Merge_circexp_norm_filtered; dim(df)
df = df[,sampleID]; dim(df)
# sort by rowSum
#df = df[order(rowSums(df),decreasing =T),]; head(df)
df = df[rowSums(df>0) >= 5,]; dim(df)  # >0 in at least 5 samples # only n = 599 remained
df = df[rowSums(df>0) >= ncol(df)*.2,]; dim(df)  # >0 in at least 20% samples # only n = 104 remained
hist(rowSums(df>0), breaks=100)
...
colnames(df)[1]=paste0("#",colnames(df)[1]) # vi to add # at the beginning
write.table(df,"~/projects/circRNA/data/QTL_BC/phenotype/circularRNA104.expression.bed", row.names=F, quote=F, sep="\t")
write.table(df[paste0("chr", df$ID) %in% circRNA_SNDA,],"~/projects/circRNA/data/QTL_BC/phenotype/circRNA104.expression.bed", row.names=F, quote=F, sep="\t")
...
# only circRNA or all circularRNA
colnames(df)[1]=paste0("#",colnames(df)[1]) # vi to add # at the beginning
write.table(df[paste0("chr", df$ID) %in% circularRNA_SNDA,],"~/projects/circRNA/data/QTL_BC/phenotype/circularRNA.PCI.bed", row.names=F, quote=F, sep="\t")
write.table(df[paste0("chr", df$ID) %in% circRNA_SNDA,],"~/projects/circRNA/data/QTL_BC/phenotype/circRNA.PCI.bed", row.names=F, quote=F, sep="\t")
