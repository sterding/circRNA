...
min.pv.gene=data.frame()
for(i in list.files("permutations", pattern="permutation.*.rds", full.names=T)){
  message(i)
  if(length(min.pv.gene)==0) min.pv.gene=readRDS(i) else min.pv.gene=cbind(min.pv.gene,readRDS(i))
}
min.pv.gene=min.pv.gene[name, ]
saveRDS(min.pv.gene, "min.pv.gene.10000permutations.rds")

min.pv.gene = readRDS("min.pv.gene.10000permutations.rds")
...
empiricalP=apply(min.pv.gene <= observedP[,1], 1, mean)  # get the percenage of min(p) <= observedP among the 10000 permutations. 
...
cat("# The estimate of the proportion of true alternative tests:", 1-qvalue(p = empiricalP)$pi0);
...
## use beta-distributon to model the minP
library(MASS) 
fitdistr(min.pv.gene[1,],"beta",list(shape1=1,shape2=1)) 
...
eGenes = genesnp %>% group_by(gene) %>% summarize(n=n(), minP=min(p.value))
eGenes=cbind(eGenes, empiricalP=empiricalP[eGenes$gene], q=q[eGenes$gene])
