...
## Log
# 20191028: change to use only neuronal healthy controls for Fig1 (59 SNDA + 43 PY = 102) 
...
### (0) only selected 102 healthy brain samples (59 SNDA + 40 TCPY + 3 MCPY)
BC102=filter(RNAseq_statistics, BRAINcode2.final.selection==1, CELL %in% c('SNDA',"TCPY","MCPY"), DX=='HC') %>% pull(SOURCE_SAMPLE_ID)
Merge_circexp_raw_filtered <- Merge_circexp_raw[,BC102]
...
saveRDS(Merge_circexp_raw_filtered, file="Merge_circexplorer_BC102.rawcount.rds")
saveRDS(annotation[annotation$ID %in% rownames(Merge_circexp_raw_filtered),], file="Merge_circexplorer_BC102.annotation.bed14.rds")
...
Merge_circexp_norm_filtered <- Merge_circexp_norm[rownames(Merge_circexp_raw_filtered), BC102]
...
# save
saveRDS(Merge_circexp_raw_filtered, file="Merge_circexplorer_BC102.filtered.rawcount.rds")
saveRDS(Merge_circexp_norm_filtered, file="Merge_circexplorer_BC102.filtered.normRPM.rds")
saveRDS(annotation_filtered, file="Merge_circexplorer_BC102.filtered.annotation.bed14.rds")
#write.table(annotation_filtered, file="Merge_circexplorer_BC102.filtered.annotation.bed14", sep = "\t", quote = F, row.names = F, col.names = F)
...
# neuronal only
Merge_circexp_raw_long_enriched.RM = filter(Merge_circexp_raw_long_enriched.RM, sampleGroup3 %in% c("PY","SN"))
...
# [1] 7471  102
# save
saveRDS(Merge_circexp_raw_filtered_and_enriched, file="Merge_circexplorer_BC102.filtered.enriched.rawcount.rds")
saveRDS(Merge_circexp_norm_filtered_and_enriched, file="Merge_circexplorer_BC102.filtered.enriched.normRPM.rds")
saveRDS(annotation_filtered_enriched, file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds")
annotation_filtered_enriched=readRDS(file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds")
write.table(annotation_filtered_enriched, file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14", sep = "\t", quote = F, row.names = F, col.names = F)
...
pdf("~/projects/circRNA/results/total_circRNA_raw_reads.BC102.pie.pdf", width=6, height = 2)
par(mfrow=c(1,3))
pie(table(readRDS("Merge_circexplorer_BC102.annotation.bed14.rds")$circType=='circRNA'),labels = NA, border='white', col=c('#fdbb84','red'),main="all")
pie(table(readRDS("Merge_circexplorer_BC102.filtered.annotation.bed14.rds")$circType=='circRNA'),labels = NA, border='white', col=c('#fdbb84','red'),main="filtered")
pie(table(readRDS("Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds")$circType=='circRNA'),labels = NA, border='white', col=c('#fdbb84','red'),main="filtered+enriched")
dev.off()

table(readRDS("Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds")[,c('geneType','circType')])
# geneType                   circRNA ciRNA
# 3prime_overlapping_ncrna       1     0
# antisense                     27     1
# lincRNA                       21     0
# polymorphic_pseudogene         1     0
# processed_transcript          14     0
# protein_coding              7342    35
# pseudogene                    28     0
# sense_intronic                 1     0

Merge_circexp_raw=readRDS("Merge_circexplorer_BC102.rawcount.rds")
Merge_circexp_raw_filtered_and_enriched=readRDS("Merge_circexplorer_BC102.filtered.enriched.rawcount.rds")
total_circRNA_raw_reads = rowSums(Merge_circexp_raw)
pdf("~/projects/circRNA/results/total_circRNA_raw_reads.BC102.pdf", width = 6, height = 5)
BREAK=200
...
## BC102
# ID circType geneName sumVar
# 1     chr2_40655612_40657444  circRNA   SLC8A1   5272
# 2     chr3_55717821_56026278  circRNA     ERC2    769
# 3     chr2_40655612_40657441  circRNA   SLC8A1    764
# 4    chr11_85707868_85714494  circRNA   PICALM    622
# 5     chr6_73005639_73043538  circRNA    RIMS1    618
# 6     chr6_73016960_73043538  circRNA    RIMS1    551
# 7     chr1_65830317_65831879  circRNA   DNAJC6    368
# 8    chr11_85707868_85742653  circRNA   PICALM    209
# 9     chr3_55768798_56026278  circRNA     ERC2    145
# 10   chr14_80130120_80164280  circRNA    NRXN3    124
# 11  chr8_105080739_105161076  circRNA    RIMS2    123
# 12   chr11_85685750_85742653  circRNA   PICALM    100
# 13  chr8_105053553_105161076  circRNA    RIMS2     89
# 14    chr2_40366540_40405633  circRNA   SLC8A1     87
# 15    chr6_73001636_73043538  circRNA    RIMS1     86
# 16   chr11_85701292_85742653  circRNA   PICALM     76
# 17    chr3_55984452_56026278  circRNA     ERC2     73
# 18    chr1_65830317_65860715  circRNA   DNAJC6     58
# 19    chr3_55922416_56041349  circRNA     ERC2     56
# 20    chr3_55922416_56026278  circRNA     ERC2     55
# 21    chr3_55717821_55984588  circRNA     ERC2     53
# 22    chr3_55733405_55922577  circRNA     ERC2     52
# 23   chr14_73614502_73614814  circRNA    PSEN1     47
# 24    chr3_55984452_56041349  circRNA     ERC2     37
# 25   chr14_73614502_73640415  circRNA    PSEN1     36
# 26    chr6_72960032_72961071  circRNA    RIMS1     35
# 27    chr9_35295692_35313986  circRNA   UNC13B     35
# 28    chr3_55922416_55984588  circRNA     ERC2     33
# 29  chr8_105105698_105161076  circRNA    RIMS2     29
# 30   chr11_85718584_85742653  circRNA   PICALM     26
# 31   chr14_73614502_73614802  circRNA    PSEN1     18
# 32    chr3_55768798_55984588  circRNA     ERC2     18
# 33  chr8_105025669_105161076  circRNA    RIMS2     18
# 34    chr6_72952016_73043538  circRNA    RIMS1     16
# 35  chr1_155207131_155209868  circRNA      GBA     13
# 36 chr11_121348826_121367758  circRNA    SORL1     12
# 37   chr11_70179575_70184559  circRNA   PPFIA1     12
# 38  chr8_104922361_105026843  circRNA    RIMS2     12
# 39  chr8_105025669_105080840  circRNA    RIMS2     12
# 40    chr6_73001636_73023375  circRNA    RIMS1     11
# 41    chr2_40387904_40405633  circRNA   SLC8A1     10
# 42   chr11_85707868_85718626  circRNA   PICALM      9
# 43   chr21_27326903_27372497  circRNA      APP      9
# 44    chr3_56173536_56183160  circRNA     ERC2      9
# 45    chr6_72960032_73043538  circRNA    RIMS1      9
# 46  chr8_104943490_105001649  circRNA    RIMS2      9
# 47  chr8_104943490_105161076  circRNA    RIMS2      9
# 48    chr9_35376024_35382504  circRNA   UNC13B      9
# 49   chr11_85707868_85726006  circRNA   PICALM      8
# 50    chr1_65830317_65871816  circRNA   DNAJC6      8
# 51   chr21_27269884_27328069  circRNA      APP      8
# 52   chr21_27326903_27354790  circRNA      APP      8
# 53   chr21_27394155_27425664  circRNA      APP      8
# 54   chr11_85707868_85712201  circRNA   PICALM      7
# 55   chr14_73614502_73664837  circRNA    PSEN1      7
# 56   chr14_79111462_79117669  circRNA    NRXN3      7
# 57   chr14_79933563_80164280  circRNA    NRXN3      7
# 58   chr21_27394155_27394358  circRNA      APP      7
# 59    chr3_55640807_55984588  circRNA     ERC2      7
# 60    chr3_55922416_56469175  circRNA     ERC2      6
# 61    chr6_72943475_73001749  circRNA    RIMS1      6
...
pdf("../results/Merge_circexplorer_BC102.filtered.enriched.nReads_vs_nSample.pdf", width = 3, height = 3)
plot(jitter(rowMeans(df>0)), jitter(rowMeans(df)), pch=20, col='#00000033', log='y')
dev.off()
...
annotation_filtered_enriched=readRDS("Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds")
...
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.circRNAs_per_hostgene.hist.pdf",width = 5, height = 3)
...
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.nCircRNA_vs_nExon.pdf", width = 3, height = 3)

## ===============================================
## exon and flanking intron length of circRNAs vs. all exons
## ===============================================
annotation_filtered_enriched=readRDS(file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds") %>% filter(circType=='circRNA') %>% dplyr::select(1:12)
...
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.length.flankingIntron.pdf", width = 2,height = 3)
...
ggplot(df,aes(x=type, y=nRepeats, col=type)) + geom_boxplot() + scale_y_log10() + theme_classic() + 
  ...
  scale_color_manual(guide=FALSE, name="type",values=c("circRNA"="red","control" = "grey"))
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.nRepeats_in_flankingIntron.pdf", width = 2,height = 3)
...
df=readRDS(file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds") %>% 
...
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.length.single-exon-circRNAs.pdf", width = 2,height = 3)

# region length distribution
readRDS(file="Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds") %>% 
...
ggsave("../results/Merge_circexplorer_BC102.filtered.enriched.lengthDistribution.pdf", width = 4,height = 3)


# lenth distribution for called circRNAs
readRDS(file="Merge_circexplorer_BC102.annotation.bed14.rds") %>% 
  ggplot(aes(x=end-start, fill=factor(circType, levels=c('ciRNA', 'circRNA')))) + 
  geom_histogram(position = 'stack',bins = 60)  + scale_color_brewer(palette="Dark2") + 
  theme_minimal()+theme_classic()+theme(legend.position="top")+ 
  scale_x_log10() + scale_fill_manual(name="circular RNA types",values=c("circRNA"="red","ciRNA" = "orange"))
ggsave("../results/Merge_circexplorer_BC102.unfiltered.lengthDistribution.pdf", width = 4,height = 3)
...
# ## ------------------------
# readRDS("Merge_circexplorer_BC102.filtered.enriched.annotation.bed14.rds") %>% filter(circType=='circRNA') %>% 
#   group_by(geneName) %>% summarise(n=n()) %>% dplyr::select(symbol=geneName, stat=n) %>% 
#   write.table(file="Merge_circexplorer_BC102.filtered.enriched.annotation.2genes.stat.xls", quote =F, sep="\t", row.names = T)
# # bash
# # ~/neurogen/pipeline/RNAseq/modules/_pathway.R -i Merge_circexplorer_BC102.filtered.enriched.annotation.2genes.stat.xls --filter no
# 
# ### tryout 3:  --- ABANDONED
...
# input_genes=read.delim("Merge_circexplorer_BC102.filtered.enriched.annotation.2genes.stat.xls", stringsAsFactors=F, row.names = 1, header=T, check.names =F)$symbol
# 
# source("~/neurogen/pipeline/RNAseq/bin/lib.R")
# gt=ORA(inputGenes=input_genes, allGenes=genes_expressed_symbol, output="Merge_circexplorer_BC102.filtered.enriched.annotation.2genes.stat")
# 
# library(dplyr)
# library(ggplot2)
...
#   ggtitle(paste("Top 10 enriched terms (p <0.01) in"), subtitle="Merge_circexplorer_BC102.filtered.enriched.annotation.2genes.stat.ORA.xls")
...
dim(genes_expressed)
# circRNA host genes
Merge_circexplorer_BC_annotation_per_cell=read.table("../results/Merge_circexplorer_BC102.annotation_per_cell.xls", sep="\t", header=T, stringsAsFactors = F); head(Merge_circexplorer_BC_annotation_per_cell)
