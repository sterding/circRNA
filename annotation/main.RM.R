...
# Merge_circexp_raw_long_filtered= ungroup(Merge_circexp_raw_long) %>%
...
#          log2fc=log2((sum(R[[1]])+1)/(sum(M[[1]])+1))) %>%
#   select(name, pvalue, log2fc) %>%
#   filter(pvalue<=0.05 & log2fc>=1)
# 
...
Merge_circexp_raw_long_filterd = filter(Merge_circexp_raw_long, R >= 2*M & R>=20) %>% 
  ...
  mutate(sampleGroup3 = ifelse(grepl('FB|PBMC', sampleGroup),'NN', ifelse(grepl('SN|MD', sampleGroup),'SN','PY'))) %>%
  select(name, sampleGroup3) %>% arrange(sampleGroup3) %>% distinct() 

Merge_circexp_raw_long_filterd %>% group_by(sampleGroup3) %>% summarise(n=n())
# # A tibble: 3 × 2
# sampleGroup3     n
# <chr> <int>
# 1           NN 19390
