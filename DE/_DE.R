...
# setwd("~/projects/circRNA/results"); input_expression_filename="../data/Merge_circexplorer_BC106.filtered.enriched.rawcount.rds"; input_covariance_filename="HCILB_SNDA.covariates.txt"; output_dir="DE_HCvsILB_SNDA"; index="hg19";
...
if(file.exists(file.path("DESeq2.RData"))) load(file.path("DESeq2.RData")) else {
  ...
  ## for circRNAs
  circRNA_annotation = read.delim("~/projects/circRNA/data/Merge_circexplorer_BC.annotation.bed14", quote="", stringsAsFactors = F);
  head(genes_annotation);dim(circRNA_annotation);
  genes_annotation = left_join(circRNA_annotation, genes_annotation, by=c("geneName" = "geneSymbol")) %>% select(geneID=ID, circType, geneSymbol = geneName, geneType, geneDescription)
  ...
  covarianceTable = mutate(covarianceTable,
                           ...
                           CONDITION=factor(CONDITION, levels = c("HC","ILB")),
                           Batch=factor(Batch),
                           Age=cut(Age, breaks = c(20,80,110)),
                           RIN=cut(RIN, breaks = c(5,8,10)),
                           PMI=cut(PMI, breaks = c(0,4,100)))
  # DEseq recommend to use only letters, numbers, and delimiters '_' or '.' for levels of factors 
  levels(covarianceTable$Age) = gsub("\\((.*),(.*)\\]","\\1_\\2",levels(covarianceTable$Age))
  levels(covarianceTable$RIN) = gsub("\\((.*),(.*)\\]","\\1_\\2",levels(covarianceTable$RIN))
  levels(covarianceTable$PMI) = gsub("\\((.*),(.*)\\]","\\1_\\2",levels(covarianceTable$PMI))
...
#sampleDists <- dist(t(vsd_adjusted_log2))
...
## PCA
#pcaData <- plotPCA(DESeqTransform(SummarizedExperiment(vsd_adjusted_log2, colData=colData(dds))),intgroup = c( "CONDITION"), returnData = TRUE)
...
if(sum(c("CONDITION", "CELL_TYPE") %in% names(covarianceTable))==1){
  ...
  apply(comparisons, 1, function(x){
    ...
    # add annotation
    res <- cbind(res, genes_annotation[match(sub("\\..*","",row.names(res)), genes_annotation$geneID), c('circType','geneSymbol','geneType','geneDescription')])
    ...
    write.table(as.data.frame(res), 
                file=file.path(paste0("DEresult.",output_dir,".all.", com_name ,".xls")), 
                sep="\t", quote =F, na="", row.names=T, col.names = NA)
    ...
    # TOADD: 
    if(!is.null(output_additonal_columns) && grepl("m", output_additonal_columns)){ # mean of raw expression values for each group, 
      baseMean_raw = as.data.frame(res[,grep("baseMean_raw", colnames(res))])
      par(pty="s"); 
      with(as.data.frame(res), plot(jitter(baseMean_raw.HC)+1, jitter(baseMean_raw.ILB)+1, 
                                    log='xy', pch=19, cex=2, cex.axis=2, cex.lab=2,
                                    xlim=range(baseMean_raw.HC, baseMean_raw.ILB)+1, 
                                    ylim=range(baseMean_raw.HC, baseMean_raw.ILB)+1, 
                                    col=ifelse(circType=="ciRNA",'orange','red')))
      with(filter(as.data.frame(res), baseMean_raw.HC>=5 | baseMean_raw.ILB>=5), text(x=1+baseMean_raw.HC, y=1+baseMean_raw.ILB, labels = geneSymbol, adj=c(0.5,0), pos=3))
    ...
    with(subset(topT, padj<=0.05 & abs(log2FoldChange)>=1), 
           text(log2FoldChange, -log10(padj), labels=geneSymbol, cex=0.5, pos=1, offset=0.2))
    ...
    rownames(topDE) = topT$geneSymbol
    annotation_row = dplyr::select(as.data.frame(topT), geneType, log2FoldChange, geneSymbol) %>% mutate(updown=ifelse(log2FoldChange>0,"up","down")) %>% select(updown, geneType)
    rownames(annotation_row) = topT$geneSymbol
    annotation_col = dplyr::select(as.data.frame(colData(dds)), CONDITION)
    ann_colors = list(
      updown = c(up = "red", down = "blue"),
      geneType = c(protein_coding = "darkblue", lincRNA = "orange", Mt_rRNA='pink', antisense='yellow', pseudogene='gray', processed_transcript='lightgray'),
      CONDITION = c(control = "green", treated = "red")
    )
    ...
    topDE=assay(vsd)[rownames(topT),]; rownames(topDE) = topT$geneSymbol
    annotation_row = dplyr::select(as.data.frame(topT), geneType, log2FoldChange, geneSymbol) %>% mutate(updown=ifelse(log2FoldChange>0,"up","down")) %>% select(updown, geneType)
    rownames(annotation_row) = topT$geneSymbol
    annotation_col = dplyr::select(as.data.frame(colData(dds)), CONDITION)
    ann_colors = list(
