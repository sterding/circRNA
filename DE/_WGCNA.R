...
# setwd("~/projects/circRNA/results"); input_expression_filename="../data/Merge_circexplorer_BC197.filtered.enriched.normRPM.rds"; input_covariance_filename="Table.PD.SNDA.pathology.covariates.xls"; output_dir="DE_SNDA"; input_annotation_filename="../data/Merge_circexplorer_BC197.filtered.enriched.annotation.bed14.rds"
...
gsg = goodSamplesGenes(datExpr, minFraction = 0.1, verbose = 3)
...
datTraits = read.table(input_covariance_filename, header = T, sep="\t", row.names = "SAMPLE_ID", check.names=F)
...
}else datAnnotation=read.delim(file.path(input_annotation_filename), header = F, stringsAsFactors =F, col.names = c("chr","start","end","ID","score",'strand','thickStart', 'thickEnd', 'itemRgb', 'exonCount', 'exonSizes', 'exonOffsets', 'circType', 'geneID', 'geneName', 'geneType'))

head(datAnnotation)

message("# You have finished uploading and formatting expression and trait data")
...
setwd(output_dir)
save(datExpr, datTraits, datAnnotation, file="WGCNA.SamplesAndTraits.RData")
#load("WGCNA.SamplesAndTraits.RData")

datTraits=datTraits[, colnames(datTraits) %in% c('CONDITION', 'CONDITION2','Braak_Braak_stage', 'MUSS', 'SEX', 'AGE', 'PMI', 'BATCH', 'RIN')]
...
softPower = min(which(SFT_r2>=0.8))
text(softPower, 0.8, labels=softPower, cex=cex1, col="black")
...
save(dynamicMods, MEList, MEs, MEDiss, METree, file= "WGCNA.Network_allSamples_signed_RLDfiltered.RData")
...
save(MEs, moduleLabels, moduleColors, geneTree, file= "WGCNA.Network_allSamples_signed_nomerge_RLDfiltered.RData")
...
system.time({foreach(module=unique(moduleColors)) %dopar% {
  ...
  write.table(datExpr[,inModule], file = paste("WGCNA.module", module, "datExpr.txt", sep="."), sep="\t", quote=F, row.names=F, col.names=T)
  
  # save gene symbol 
  write.table(data.frame(ID=modGenes, geneName=datAnnotation$geneName[match(modGenes,datAnnotation$ID)]), 
              file = paste("WGCNA.module", module, "anno.txt", sep="."), sep="\t", quote=F, row.names=F, col.names=T)
  
  # Select the corresponding TOM
  modTOM = TOM[inModule, inModule];
  dimnames(modTOM) = list(modGenes, modGenes)
  # Export the network into an edge list file VisANT can read
  vis = WGCNA::exportNetworkToVisANT(modTOM,
                              file = paste("WGCNA.module", module, "VisANTInput.txt", sep="."),
                              weighted = TRUE,
                              threshold = 0,
  ...
  #                             file = paste("WGCNA.module.", module, ".VisANTInputTop30.txt", sep=""),
  #                             weighted = TRUE,
  #                             threshold = 0,
  #                             probeToGene = data.frame(datAnnotation$ID, datAnnotation$geneName) )
...
MEs = orderMEs(MEs0)
moduleTraitCor = WGCNA::cor(MEs, datTraits2number, use= "pairwise.complete.obs", method="spearman")  # <=== FIX THIS https://www.biostars.org/p/293281/
